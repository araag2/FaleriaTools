...
class CharacterCreatorPage {
	...
	async pInit () {
		try {
			...
			this._$statGenHost = $(`<div></div>`);
			this._statGenUi.render(this._$statGenHost);

			this._render();
			window.dispatchEvent(new Event("toolsLoaded"));
		} catch (e) {
			$(`#character-creator-main`).empty().append($(`<div class="ve-flex-vh-center initial-message initial-message--large w-100 h-100">Failed to load Character Creator. ${VeCt.STR_SEE_CONSOLE}</div>`));
			setTimeout(() => { throw e; });
		}
	}
	...
	async _pLoadClasses () {
		const classes = /** @type {*} */ (await DataUtil.class.loadJSON());
		const prereleaseClasses = /** @type {*} */ (await DataUtil.class.loadPrerelease());
		const brewClasses = /** @type {*} */ (await DataUtil.class.loadBrew());

		return [
			...(classes?.class || []),
			...(prereleaseClasses?.class || []),
			...(brewClasses?.class || []),
		]
			.filter(it => {
	...
	_render () {
		const $root = $(`#character-creator-main`).empty();
		...
		const $steps = $(`<div class="character-creator__steps"></div>`);
		steps.forEach(step => {
			const isActive = this._state.activeStep === step.id;
			const $btn = $(`<button class="ve-btn ${isActive ? "ve-btn-primary" : "ve-btn-default"} character-creator__step"></button>`)
				.on("click", () => {
					this._state.activeStep = step.id;
					this._render();
				});
			$btn.append($(`<div class="character-creator__step-title">${step.title}</div>`));
			$btn.append($(`<div class="character-creator__step-sub">${step.sub}</div>`));
			$steps.append($btn);
		});

		const $panel = $(`<div class="character-creator__panel"></div>`);
		this._renderActivePanel($panel);

		const $layout = $(`<div class="character-creator__layout"></div>`);
		$layout.append($steps);
		$layout.append($panel);
		$root.append($layout);
	}
	...
	_renderOverviewStep ($panel) {
		$panel.append($(`<h3 class="mt-0">Character Creator</h3>`));
		$panel.append($(`<p>Build your character in guided steps inspired by modern VTT creators. This flow uses site content data for races, classes, backgrounds, and feats.</p>`));
		$panel.append($(`<p><b>Stat Allocation</b> is powered by the built-in Point Buy system on the Ability Scores step.</p>`));
		$panel.append($(`<p>Use the step list on the left to progress and revisit choices at any time.</p>`));
	}

	_renderRaceStep ($panel) {
		const selected = this._state.ixRace == null ? null : this._races[this._state.ixRace];
		$panel.append($(`<h3 class="mt-0">Choose a Race</h3>`));
		...
		const $list = $(`<div class="character-creator__list"></div>`);
		races.forEach((race, ix) => {
			const actualIx = this._races.indexOf(race);
			const isSel = actualIx === this._state.ixRace;
			const $card = $(`<div class="character-creator__card"></div>`);
			$card.append($(`<div><b>${race.name}</b></div>`));
			$card.append($(`<div class="ve-muted ve-small">${Parser.sourceJsonToAbv(race.source)}</div>`));
			$card.append($(`<button class="ve-btn ve-btn-xs ${isSel ? "ve-btn-primary" : "ve-btn-default"} mt-2">${isSel ? "Selected" : "Select"}</button>`)
				.on("click", () => {
		...
		if (selected) {
			$panel.append($(`<div class="character-creator__summary"><b>Selected:</b> ${selected.name}</div>`));
		}
	}

	_renderClassStep ($panel) {
		const cls = this._state.ixClass == null ? null : this._classes[this._state.ixClass];
		$panel.append($(`<h3 class="mt-0">Choose a Class</h3>`));
		$panel.append(this._$getSearchBox({
			prop: "searchClass",
		...
		classes.forEach(c => {
			...
			const $card = $(`<div class="character-creator__card"></div>`);
			$card.append($(`<div><b>${c.name}</b></div>`));
			$card.append($(`<div class="ve-muted ve-small">${Parser.sourceJsonToAbv(c.source)}</div>`));
			$card.append($(`<button class="ve-btn ve-btn-xs ${isSel ? "ve-btn-primary" : "ve-btn-default"} mt-2">${isSel ? "Selected" : "Select"}</button>`)
				.on("click", () => {
		...
		if (cls?.subclasses?.length) {
			$panel.append($(`<hr class="hr-3">`));
			$panel.append($(`<h4>Subclass</h4>`));
			const subclasses = cls.subclasses;
			const $subWrp = $(`<div class="character-creator__list"></div>`);
			subclasses.forEach(sc => {
				const ixSc = cls.subclasses.indexOf(sc);
				const isSel = ixSc === this._state.ixSubclass;
				const $card = $(`<div class="character-creator__card"></div>`);
				$card.append($(`<div><b>${sc.name}</b></div>`));
				$card.append($(`<div class="ve-muted ve-small">${Parser.sourceJsonToAbv(sc.source)}</div>`));
				$card.append($(`<button class="ve-btn ve-btn-xs ${isSel ? "ve-btn-primary" : "ve-btn-default"} mt-2">${isSel ? "Selected" : "Select"}</button>`)
					.on("click", () => {
	...
	_renderBackgroundStep ($panel) {
		...
		$panel.append($(`<h3 class="mt-0">Choose a Background</h3>`));
		$panel.append(this._$getSearchBox({
		...
		backgrounds.forEach(bg => {
			...
			const $card = $(`<div class="character-creator__card"></div>`);
			$card.append($(`<div><b>${bg.name}</b></div>`));
			$card.append($(`<div class="ve-muted ve-small">${Parser.sourceJsonToAbv(bg.source)}</div>`));
			$card.append($(`<button class="ve-btn ve-btn-xs ${isSel ? "ve-btn-primary" : "ve-btn-default"} mt-2">${isSel ? "Selected" : "Select"}</button>`)
				.on("click", () => {
		...
		if (selected) $panel.append($(`<div class="character-creator__summary"><b>Selected:</b> ${selected.name}</div>`));
	}

	_renderFeatStep ($panel) {
		const selected = this._state.ixFeat == null ? null : this._feats[this._state.ixFeat];
		$panel.append($(`<h3 class="mt-0">Choose a Feat (Optional)</h3>`));
		const $top = $(`<div class="ve-flex-v-center mb-2"></div>`);
		$top.append(this._$getSearchBox({
			prop: "searchFeat",
			placeholder: "Search feats...",
		}));
		$top.append($(`<button class="ve-btn ve-btn-xs ve-btn-default ml-2">Clear</button>`)
			.on("click", () => {
		...
		feats.forEach(ft => {
			...
			const $card = $(`<div class="character-creator__card"></div>`);
			$card.append($(`<div><b>${ft.name}</b></div>`));
			$card.append($(`<div class="ve-muted ve-small">${Parser.sourceJsonToAbv(ft.source)}</div>`));
			$card.append($(`<button class="ve-btn ve-btn-xs ${isSel ? "ve-btn-primary" : "ve-btn-default"} mt-2">${isSel ? "Selected" : "Select"}</button>`)
				.on("click", () => {
	...
	_renderStatsStep ($panel) {
		$panel.append($(`<h3 class="mt-0">Ability Scores (Point Buy)</h3>`));
		$panel.append($(`<p class="ve-muted">This section uses the built-in Stat Generator Point Buy system.</p>`));
	...
	_renderSummaryStep ($panel) {
		...
		$panel.append($(`<h3 class="mt-0">Character Summary</h3>`));
		$panel.append($(`<div class="mb-1"><b>Race:</b> ${race ? `${race.name} (${Parser.sourceJsonToAbv(race.source)})` : "Not selected"}</div>`));
		$panel.append($(`<div class="mb-1"><b>Class:</b> ${cls ? `${cls.name} (${Parser.sourceJsonToAbv(cls.source)})` : "Not selected"}</div>`));
		$panel.append($(`<div class="mb-1"><b>Subclass:</b> ${sc ? `${sc.name} (${Parser.sourceJsonToAbv(sc.source)})` : "Not selected"}</div>`));
		$panel.append($(`<div class="mb-1"><b>Background:</b> ${bg ? `${bg.name} (${Parser.sourceJsonToAbv(bg.source)})` : "Not selected"}</div>`));
		$panel.append($(`<div class="mb-3"><b>Feat:</b> ${feat ? `${feat.name} (${Parser.sourceJsonToAbv(feat.source)})` : "Not selected"}</div>`));

		$panel.append($(`<h4>Ability Scores (Point Buy)</h4>`));
		$panel.append($(`<div><b>STR</b> ${pbTotals.str ?? "\u2014"} | <b>DEX</b> ${pbTotals.dex ?? "\u2014"} | <b>CON</b> ${pbTotals.con ?? "\u2014"} | <b>INT</b> ${pbTotals.int ?? "\u2014"} | <b>WIS</b> ${pbTotals.wis ?? "\u2014"} | <b>CHA</b> ${pbTotals.cha ?? "\u2014"}</div>`));
	}

	_$getSearchBox ({prop, placeholder}) {
		return $(`<input class="form-control mb-2" type="search" placeholder="${placeholder}">`)
			.val(this._state[prop] || "")
			.on("input", evt => {
				const target = evt.currentTarget;
				this._state[prop] = target instanceof HTMLInputElement ? target.value : "";
				this._render();
			});
	}
}
